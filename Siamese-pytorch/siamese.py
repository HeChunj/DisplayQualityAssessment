...
class Siamese(object):
    ...
    def generate(self):
        # 载入模型与权值
        print('Inferencing, loading weights into state dict...')
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        if "vgg16" == self.model_name:
            siamese = SiameseVgg
        elif "resnet50" == self.model_name:
            siamese = SiameseResnet
        elif "vit" == self.model_name:
            siamese = SiameseVIT
    ...
    def detect_image(self, image_1, image_2):
        ...
        with torch.no_grad():
            ...
            # ---------------------------------------------------#
            photo_1 = torch.from_numpy(np.expand_dims(
                np.transpose(photo_1, (2, 0, 1)), 0)).type(torch.FloatTensor)
            photo_2 = torch.from_numpy(np.expand_dims(
