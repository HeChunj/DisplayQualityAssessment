...
if __name__ == "__main__":
    ...
    image_folder = ""
    dmos_file_train = "/home/hechunjiang/gradio/GeoFormer/finetune_data/40_finetune_data_baipingheng.csv"
    dmos_file_test = "/home/hechunjiang/gradio/GeoFormer/finetune_data/all_test_finetune_data_baipingheng.csv"
    #----------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    save_dir            = 'logs-finetune-resnet-baipingheng'
    #------------------------------------------------------------------#
    ...
    for name, param in model.named_parameters():
        if "fully_connect1" in name or "fully_connect2" in name or "fully_connect3" in name:
            param.requires_grad = True
        else:
            param.requires_grad = False

    # summary(model, input_size=[(3, 256, 256), (3, 256, 256)], device='cpu')
    ...
    dataset_test = KADID10kDataset(input_shape=input_shape,
                              image_folder=image_folder, dmos_file=dmos_file_test, random=True, autoaugment_flag=True)

    num_train = len(dataset_train)
    num_val = len(dataset_test)
    # 将数据集分割成训练集和验证集
    # num_train = int(train_ratio * len(dataset))
    # num_val = len(dataset) - num_train
    
    # dataset_train, dataset_test = random_split(dataset, [num_train, num_val])
    ...
    if True:
        ...
        gen_val         = DataLoader(dataset_test, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True,
