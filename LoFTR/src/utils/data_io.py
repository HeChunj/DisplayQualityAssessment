...
def read_im(im_path, imsize=None, dfactor=1, value_to_scale=min):
    ...
    wt, ht, scale = resize_im(wo, ho, imsize=imsize,
                              dfactor=dfactor, value_to_scale=value_to_scale)
...
def load_gray_scale_tensor(im_path, device, imsize=None, dfactor=1, value_to_scale=max):
    im_rgb, scale = read_im(
        im_path, imsize, dfactor=dfactor, value_to_scale=value_to_scale)
...
def load_gray_scale_tensor_cv(im_path, device, imsize=None, dfactor=1, enhanced=False, value_to_scale=min, aspan=False):
    ...
    wt, ht, scale = resize_im(
        wo, ho, imsize=imsize, dfactor=dfactor, value_to_scale=value_to_scale, aspan=aspan)
...
def load_im_tensor(im_path, device, imsize=None, normalize=True,
    ...
    if with_gray:
        ...
        gray = np.array(im_rgb.convert('L'))
        if not raw_gray:
            gray = transforms.functional.to_tensor(
                gray).unsqueeze(0).to(device)
